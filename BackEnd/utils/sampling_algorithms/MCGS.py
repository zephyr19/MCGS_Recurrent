...
class MCGS(object):
    def __init__(self, param_settings):
        self.__alpha = param_settings['alpha']
        self.__beta = param_settings['beta']
        self.__loss_weight = np.array(param_settings['loss weight']) / sum(
            param_settings['loss weight'])
    ...
    def __identify_pivot_and_star_structures(self, G):
        def __DFS(current_node, father_node=None, grandfather_node=None):
            ...
            for other_node in self.__G_neighbors_dict[current_node]:
                if other_node == grandfather_node:
                    ...
                    continue
                elif father_node and (
                        other_node in self.__G_neighbors_dict[father_node]):
                    triangle_node_set.add(current_node)
                    triangle_node_set.add(father_node)
                    triangle_node_set.add(other_node)
                elif father_node and (
                        other_node not in self.__G_neighbors_dict[father_node]):
                    continue
                else:
        ...
        for node in nodes_average:
            if (node not in seen) or (node not in triangle_node_set): __DFS(node)
        minority_structures['star'] = [_ for _ in nodes_average if
                                       _ not in triangle_node_set]
    ...
    def __majority_structure_sampling(self, G, Gs, size):
        ...
        while len(Gs) < size:
            ...
            for iter_node in candidate_node_set:
                ...
                new_MSE = temp_MSE + (self.__G_node_degree_dict[iter_node] - len(
                    new_edge_nodes)) ** 2
                if self.__loss_weight[2] == 0 or not self.__G_neighbors_dict[
                    iter_node]:
                    new_JI = 0
                else:
                    new_JI = temp_JI + len(new_edge_nodes) / len(
                        self.__G_neighbors_dict[iter_node])

                for other_node in new_edge_nodes:
                    new_MSE += - 2 * sub_degrees[other_node] + 1
                    if self.__loss_weight[2] == 0 or self.__G_node_degree_dict[
                        other_node] == 0:
                        new_JI += 0
                    else:
                ...
                if self.__loss_weight[1] == 0:
                    ...
                    new_NCC = (
                                          temp_connected_components + 1) / G_connected_components
            ...
            if self.__loss_weight[0] == 0 and self.__loss_weight[1] == 0:
                ...
                for key_node, value in all_node_record_dict.items():
                    MSE_detal = max_loss_record_dict['MSE'] - \
                                min_loss_record_dict['MSE']['value']
                    NCC_detal = max_loss_record_dict['NCC'] - \
                                min_loss_record_dict['NCC']['value']
                    JI_detal = max_loss_record_dict['JI'] - \
                               min_loss_record_dict['JI']['value']
                    try:
                        if self.__loss_weight[0] == 0:
                            loss_MSE_value = 0
                        else:
                            loss_MSE_value = (value['MSE'] -
                                              min_loss_record_dict['MSE'][
                                                  'value']) / MSE_detal
                        ...
                        if self.__loss_weight[2] == 0:
                            ...
                            loss_JI_value = (value['JI'] -
                                             min_loss_record_dict['JI'][
                                                 0]) / JI_detal

                        # calculate the loss function of the current node and update the minimal loss record
                        current_loss_function_value = loss_MSE_value * \
                                                      self.__loss_weight[0] + \
                                                      loss_NCC_value * \
                                                      self.__loss_weight[1] + \
                                                      loss_JI_value * \
                                                      self.__loss_weight[2]
    ...
    def __delete_node_randomly(self, Gs, size):
        ...
        no_minority_structure_nodes = list(
            Gs_nodes - self.__G_minority_structures_set)
